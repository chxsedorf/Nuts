...
export default function Page() {
  ...
  function placeCard(row: number, col: number) {
    ...
    if (gainedScore > 0) {
      ...
      window.setTimeout(() => {
        setScorePopup((current) =>
          current?.id === popupId ? null : current
        );
      }, 900);
    }
    ...
    if (hasClearHand) {
      /**
       * 重要:
       * 役判定は lib/handJudge.ts の judgeHandsAfterPlace だけを使う。
       * ここでは判定ロジックを一切変更せず、返ってきた hand.cards を
       * 演出後に確実に消すだけにする。
       */
      const boardAfterClear = nextBoard.map((line) => [...line]);
      ...
      setIsResolving(true);
      setClearingCells(new Set(clearKeys));

      window.setTimeout(() => {
        setBoard(boardAfterClear);
        setClearingCells(new Set());
        setIsResolving(false);
        finishGameIfBoardFull(boardAfterClear, finalScore);
      }, 430);
    } else {
            ...
            <div className="mt-5 grid w-full gap-3">
              ...
              {isDebugMode ? (
                <button
                  ...
                  className="select-none touch-manipulation rounded-2xl border border-[#D6B36A]/25 bg-[#D6B36A]/10 px-5 py-4 text-sm font-black uppercase tracking-[0.22em] text-[#D6B36A] transition hover:bg-[#D6B36A]/15 active:scale-[0.99]"
                >
                  Dev Mode
                </button>
              ) : null}
      ...
      <div className={["relative flex h-[100dvh] flex-col gap-3 px-3 transition duration-500 lg:hidden", runStartPulse ? "opacity-0 scale-[0.99]" : "opacity-100 scale-100"].join(" ")}
        ...
        <footer className="flex h-[48px] shrink-0 items-center justify-start gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-3 backdrop-blur-xl">
          ...
          {isDebugMode ? (
            <button
              onClick={() => setDevModeOpen(true)}
              className="flex h-9 select-none touch-manipulation items-center rounded-xl border border-[#D6B36A]/25 bg-[#D6B36A]/10 px-3 text-[10px] font-black uppercase tracking-[0.16em] text-[#D6B36A]"
            >
              Dev
            </button>
          ) : null}
        ...
        <aside className="rounded-[30px] border border-white/10 bg-white/[0.035] p-7 shadow-[0_24px_80px_rgba(0,0,0,0.32)] backdrop-blur-xl">
          ...
          {isDebugMode ? (
            <button
              onClick={() => setDevModeOpen(true)}
              className={[
                "mt-3 flex w-full items-center justify-center gap-3 rounded-2xl border border-[#D6B36A]/25",
                "bg-[#D6B36A]/10 px-5 py-4 text-sm font-black uppercase tracking-[0.2em] text-[#D6B36A]",
