  ...
  return (
    ...
    >
      ...
      <div className={["relative flex h-[100dvh] flex-col gap-2 px-3 transition duration-500 lg:hidden", runStartPulse ? "opacity-0 scale-[0.99]" : "opacity-100 scale-100"].join(" ")}
        ...
        <section
          className={[
            "flex min-h-0 flex-1 items-center justify-center rounded-[28px] border border-white/10 bg-white/[0.025] p-0 backdrop-blur-xl transition duration-300",
          ].join(" ")}
        >
          <div className="grid aspect-square w-full max-h-full max-w-full grid-cols-5 grid-rows-5 gap-1">
            {board.map((line, row) =>
              line.map((cell, col) => {
                const key = `${row}-${col}`;
                ...
                return (
                  <button
                    ...
                    className={[
                      "group relative h-full w-full select-none touch-manipulation overflow-hidden rounded-[18px] border transition duration-200",
                  ...
                  >
                    ...
                    {cell ? (
                      <div
                        className={[
                          "absolute inset-[2px] transition-opacity duration-200",
